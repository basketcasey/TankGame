...
public class Tank {
    ...
    public Tank(int xSize, int ySize, int origPosX, int origPosY, Wall wall) {
        this.wall = wall;
...
//            if (angle >= 2*Math.PI) { angle = 0 + (angle + 2*Math.PI); }
//            if (angle < 0) { angle = 2*Math.PI - (angle - 2*Math.PI); }
